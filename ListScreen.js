...
export default function ListScreen({ route, navigation }) {
  ...
  const fetchMovies = async (pageNum) => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`https://ophim1.com/v1/api/danh-sach/${slug}?page=${pageNum}`);
      const newItems = res.data.data.items || [];
      setMovies((prev) => [...prev, ...newItems]);
      const { currentPage, pageRanges } = res.data.data.params.pagination;
      setHasMore(currentPage < pageRanges);
      setPage(currentPage);
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Danh mục: {name}</Text>
      <FlatList
        data={movies}
